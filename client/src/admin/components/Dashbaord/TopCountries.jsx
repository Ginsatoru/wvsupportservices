    ...
    return (
      ...
      >
        ...
        <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">
          Visitors: <span className="font-bold text-blue-600 dark:text-blue-400">{data.value.toLocaleString()}</span>
        </p>
        <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">
      ...
      {payload.map((entry, index) => (
        ...
        >
          ...
          <span className="text-xs font-medium flex items-center gap-1">
            ...
            <span className={`truncate max-w-16 sm:max-w-none ${darkMode ? "text-gray-300" : "text-gray-700"}`}>
              {entry.value}
            </span>
          </span>
        </div>
...
const TopCountries = ({ data = [], colors = COLORS, darkMode = false }) => {
  ...
  return (
    <div
      className={`relative p-4 sm:p-6 lg:p-8 rounded-xl sm:rounded-2xl transition-all duration-300 hover:shadow-xl w-full max-w-full overflow-hidden ${
        darkMode
    ...
    >
      ...
      <div className="relative w-full">
        ...
        >
          <PieChart>
            ...
            <Legend content={<CustomLegend darkMode={darkMode} payload={formattedData.map((item, index) => ({
              value: item.name,
      ...
      {formattedData.length === 0 && (
        <div className="flex items-center justify-center h-32 sm:h-40">
          <p className={`text-sm sm:text-base text-gray-500 ${darkMode ? "text-gray-400" : "text-gray-500"}`}>
